...
class NLPReader {
  ...
  /*
   * Read the documents, and populate the entityKBA/entityIds/entityHeaders
   * and relationKBA/relationIds to be filled up later
   */
        ...
        for (s <- d.sentences; r <- s.relations;
          ...
          for (e1id <- entityId(e1); e2id <- entityId(e2)) {
            ...
            for (unnormRel <- r.relations; rel = normalizeType(unnormRel)) {
              if (!db._relationKBA.contains(rid)) {
                db._relationIds += rid
                db._relationKBA(rid) = EntityUtils.emptyKBA(rid._1 + "|" + rid._2)
              }
  ...
  /*
  * Fill entityHeader, entityInfo and entityFreebase
  */
    ...
    if (useMongo) {
      ...
      for (ehl <- ehf; eil = eif.next(); efl = eff.next()) {
        ...
        if (db._entityHeader.contains(mid)) {
          val oh = db._entityHeader(mid)
          db._entityHeader(mid) = EntityHeader(eh.id, eh.name, eh.nerTag, oh.popularity, eh.geo)
  ...
  */
  ...
  def read(word: Option[String] = None): DB = {
    val db = new InMemoryDB
    val cfg = ConfigFactory.load()
    val baseDir = cfg.getString("nlp.data.baseDir") //.replaceAll(" ", "\\ ")
    db.clear()
    readDocs(baseDir + "/docs.nlp.flr.json.gz", db, word)
    //readStaleness(baseDir + "/docs.staleness.json.gz", db)
    addRelationInfo(db)
