...
class NLPReader {
  ...
  def readDocs(docsFile: String, db: InMemoryDB): Unit = {
    ...
    for (eid <- db.entityIds) {
      ...
      val ments = einfo.mentions(eid)
      val name = ments.map(_.text).groupBy(x => x).map(p => p._1 -> p._2.size).maxBy(_._2)._1
      val ner = ments.flatMap(_.ner.toSeq).groupBy(x => x).map(p => p._1 -> p._2.size).maxBy(_._2)._1
      db._entityHeader(eid) = EntityHeader(eid, name, ner, ments.size.toDouble)
    }
  }
  ...
  def read: DB = {
    ...
    readDocs(baseDir + "/nigeria_dataset_v04.nlp.lr.json.gz", db)
    db
  }
